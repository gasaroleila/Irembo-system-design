...
export function EditCheckIn(props:any): JSX.Element {
   ...
   const editCheckin: SubmitHandler<CheckIn> = async(data) => {
      data.asset_checkout = assetId
      console.log('checkout',data.asset_checkout)
      handleLoading(true)
      let newCheckin = {
          asset_checkout: data.asset_checkout == null ? checkin.asset_checkout?.id : data.asset_checkout,
          checkin_date: data.checkin_date || checkin.checkin_date,
          reason: data.reason || checkin.reason,
          comment: data.comment || checkin.comment
      }
       console.log(newCheckin)
         const checkinService = new CheckinService()
         try{
         const response = await checkinService.update(newCheckin,props.id)
   ...
   return (
      ...
      {/* toast ends here */}
         <form onSubmit={handleSubmit(editCheckin)} className="text-sm w-full -mt-2">
            <div className=" mt-7 w-full grid grid-cols-1 lg:grid-cols-2 gap-5">
               ...
               <div className="form-group">
                  ...
                  <input type="text" name="" id=""
                     ...
                     // value={search}
                     defaultValue={checkin.asset_checkout?.asset?.name}
               ...
               <div className="form-group w-full">
                  <label htmlFor="checkin_date" className="mb-2 text-sm capitalize block">Checkin date</label>
                  <input
                     type="date"
                     id="checkin_date"
            ...
            <div className=" mt-7 w-full flex flex-col">
               ...
               <textarea id="reason"  rows={3} className="rounded-sm bg-gray-50 ring-1 ring-gray-200 outline-none w-full py-2 px-3" defaultValue={checkin.reason}
                {...register('reason',
                   )}
             >
            ...
            <div className=" mt-7 w-full flex flex-col">
               ...
               <textarea id="comment"  rows={5} className="rounded-sm bg-gray-50 ring-1 ring-gray-200 outline-none w-full py-2 px-3" defaultValue={checkin.comment}
                {...register('comment',
                   )}
             >
